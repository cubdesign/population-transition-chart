...
import { getPrefectures, ApiPrefectureResponse } from "@/services/resasApi";
...
describe("resasApi getPrefectures", () => {
  it("レスポンスが正しく返る", async () => {
    const wont: ApiPrefectureResponse = {
      message: null,
      result: [
    ...
    expect(await getPrefectures()).toEqual(wont);
  });
});
